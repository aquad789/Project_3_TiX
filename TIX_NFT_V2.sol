...
// Setting imports - Context, Access Control, ERC721, Utilities
...
contract TIXNFT is Context, AccessControl, ERC721 {
...
// Using a Struct to create a record for NFT details. This will be used later to track sales & analyze hypothetical revenues
...
// Constructing the NFT Ticket - ERC721

    constructor(
        string memory TIXName,
        string memory TIXSymbol,
        uint256 ticketPrice,
        uint256 totalSupply,
        address organiser
    ) public ERC721(TIXName, TIXSymbol) {
...
// Using a Modifier to Limit Ticket Sales (Total Supply) to Venue Capacity 
...
// Modifying contract to only allow owner of the contract (event organizer) to mint new tickets 
...
// Creating a function to cap secondary sales prices at 110% of the original purchase price of Ticket 
...
// Mint new tickets and assign it to operator, Access controlled by minter only, Returns new ticketId
...
// Bulk mint specified number of tickets to assign it to a operator. Modifier to check the ticket count is less than total supply
...
// Primary purchase for the tickets. Adds new customer if not exists. Adds buyer to tickets mapping. Update ticket details
...
// Secondary purchase for the tickets. Modifier to validate that the selling price shouldn't exceed 110% of purchase price for peer to peer transfers. Adds new customer if not exists
...
// Add ticket for sale with its details. Validate that the selling price shouldn't exceed 110% of purchase price. Organiser can not use secondary market sale
